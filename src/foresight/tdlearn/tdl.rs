...
impl<'m, S, A, F, R> Handler<&'m Transition<S, A>>
    for TDLambda<F, Tr<&'m S, F, R>>
where
    ...
    fn handle(
        ...
        match transition.to {
            ...
            Observation::Full(ref to) | Observation::Partial(ref to) => {
                let td_error = transition.reward
                    + self.gamma * self.fa_theta.evaluate((to,))
                    - pred;
