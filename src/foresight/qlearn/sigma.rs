use crate::{
    ...
    utils::argmaxima, Enumerable, Function, Handler, Parameterised,
};
use rand::thread_rng;
...
impl<S, Q, P> QSigma<S, Q, P> {
    fn update_backup(
        &mut self,
        entry: BackupEntry<S>,
    ) -> Option<Result<Q::Response, Q::Error>>
    where
...
where
    ...
    <P as Function<(&'m S,)>>::Output:
        Index<usize, Output = f64> + IntoIterator<Item = f64>,
    <<P as Function<(&'m S,)>>::Output as IntoIterator>::IntoIter:
        ExactSizeIterator,
{
    ...
    fn handle(
        ...
        let res = if t.terminated() {
            ...
            let residual = t.reward
                + self.gamma
                    * (self.sigma * nqsna + (1.0 - self.sigma) * exp_nqs)
                - qa;
