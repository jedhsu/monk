...
use crate::{
    ...
    params::{Thinking, Weights, WeightsView, WeightsViewMut},
...
pub struct Patience<P1, P2> {
    pub left: P1,
    pub right: P2,
}

impl<P1, P2> Algorithm for Patience<P1, P2>
where
    P1: Algorithm,
    P2: Algorithm,
{
...
impl<'s, S, A, P1, P2> Function<(&'s S, A)> for Patience<P1, P2>
where
...
impl<'s, S, A, P1, P2> Differentiable<(&'s S, A)> for Patience<P1, P2>
where
    A: std::borrow::Borrow<(P1::Action, P2::Action)>,
...
impl<'s, S, P1, P2> Policy<&'s S> for Patience<P1, P2>
where
    P1: Policy<&'s S>,
    P2: Policy<&'s S>,
{
...
type Relay<'s, 'a, S, P> =
    StateActionUpdate<&'s S, &'a <P as Policy<(&'s S,)>>::Action>;
...
impl<'s, 'a, S, P1, P2> Handler<Message<'s, 'a, S, P1, P2>> for Patience<P1, P2>
where
...
impl<'m, D, P1, P2> Handler<GradientUpdate<&'m ArrayBase<D, Ix2>>>
    for Patience<P1, P2>
where
    D: Data<Elem = f64>,
    P1: Thinking + Handler<GradientUpdate<ArrayView2<'m, f64>>>,
    P2: Thinking + Handler<GradientUpdate<ArrayView2<'m, f64>>>,
{
    type Response = Response<P1::Response, P2::Response>;
    type Error = Error<P1::Error, P2::Error>;
...
impl<'m, D, P1, P2> Handler<ScaledGradientUpdate<&'m ArrayBase<D, Ix2>>>
    for Patience<P1, P2>
where
    D: Data<Elem = f64>,
    P1: Thinking + Handler<ScaledGradientUpdate<ArrayView2<'m, f64>>>,
    P2: Thinking + Handler<ScaledGradientUpdate<ArrayView2<'m, f64>>>,
{
    type Response = Response<P1::Response, P2::Response>;
    type Error = Error<P1::Error, P2::Error>;
